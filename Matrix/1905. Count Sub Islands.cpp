...
public:
    ...
    bool checkSubIsland(vector<vector<int>> &grid1, vector<vector<int>> &grid2, int i, int j)
    {
        ...
        {
            return true; // base condition to check out of bound
        } // traversal complete i.e. it checked all the grids so it will return true
...
public:
    ...
    bool checkSubIsland(vector<vector<int>> &grid1, vector<vector<int>> &grid2, int i, int j)
    {
        ...
        while (!que.empty())
        {
            ...
            if (grid1[x][y] != 1)
            { // grid1 must have 1 at that same co-ordinate
                result = false;
            }
